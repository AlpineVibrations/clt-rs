...
*   Before spawning or releasing a fresh Git-enabled Codex process, require the intended checkout, branch, and upstream configuration to be complete and require the index to match `HEAD`. Perform the safe fast-forward-only startup sync unless an older `WORKING` journal depends on the current history; preserve that commit instead of invalidating the older proof boundary. If the task board is dirty, create a dedicated `CLT Agent` prelaunch checkpoint containing the complete `tasks/` tree while preserving unrelated unstaged and untracked work. Capture the resulting exact `HEAD`, branch, worktree baseline, and upstream configuration. Persist this server-owned launch state before release; any spawned child remains gated until its remaining session fences are registered, and preparation or persistence failure must prevent Codex from executing. Recheck the frozen state when Todo moves to Doing and bind it to a `WORKING` journal before changing the board. The selected task's durable identity must exist exactly once in the checkpointed starting commit.
...
*   At `clt done`, require the implementation and active Doing task to be staged, project the selected task's move into Done in a private index, reject staged changes elsewhere in the task board, and seal the exact resulting full repository tree. Permit concurrent task-board edits to remain unstaged and preserve them in the worktree; they cannot enter the commit because the exact staged tree remains authoritative. Accept only the one-parent CLT task commit whose complete tree, parent boundary, task identity, author/committer identity, and trailer match that seal; a path list or semantically similar rewrite is not sufficient proof.
...
*   Treat a human move of a session-linked managed task to Done as explicit acceptance of external completion only while its journal remains `WORKING`. Atomically require no active worker, live session, or project lease; take a short project fence; cancel the obsolete proof journal; clear its idle resume control; then move the task and release the fence. Preserve the session marker and record cancellation instead of representing the external work as a proven managed commit. Never use this override after the manifest is sealed or publication is pending.
