...
The shared Git index is different from the shared worktree: it is a cooperative ownership boundary during automated finalization. People, interactive sessions, and parallel tools must not stage or unstage changes while the active automated task owns the index. CLT seals the reviewed index without requiring unstaged or untracked work to match the launch baseline; later outside edits are intentional shared work. Git cannot identify which actor staged a change.
...
- **Stopped Tasks**: Skip tasks displayed as `[STOPPED]` or ending with `clt:stopped`, including during blocked-task recovery. The user can press `s` on an unlinked task to stop it or allow it to start again. Preserve the marker until the user restarts the task.
