...
impl MappedSharedWalCoordination {
    ...
    pub(crate) fn repair_transient_state_for_exclusive_open(&self) {
        ...
        // An OFD lock probe succeeds on the same open file description even
        // when one of our sibling connections still owns that reader slot.
        // Keep registration and release excluded through the probe and clear.
        let local = self.local_lock_state.lock();
...
mod tests {
    ...
    #[test]
    fn mapped_shared_wal_coordination_repair_preserves_same_mapping_reader_slots() {
        for process_scoped in [false, true] {
            ...
            mapped.record_frame(9, 4);
            let first = mapped
                .register_reader_for_snapshot(mapped.owner_record(), 4)
                .unwrap();
            let sibling = mapped
                .register_reader_for_snapshot(mapped.owner_record(), 4)
                .unwrap();
            ...
            assert_eq!(mapped.min_active_reader_frame(), Some(2));
            assert_eq!(mapped.find_frame(7, 0, 4, None), Some(2));
            assert_eq!(mapped.find_frame(9, 0, 4, None), Some(4));

            mapped.unregister_reader(older);
            mapped.unregister_reader_for_snapshot(first);
            assert_eq!(mapped.reader_owner(sibling.slot_index), Some(sibling.owner));
            assert_eq!(mapped.min_active_reader_frame(), Some(4));
            mapped.unregister_reader_for_snapshot(sibling);
            assert_eq!(mapped.min_active_reader_frame(), None);
            assert_eq!(mapped.find_frame(7, 0, 4, None), Some(2));
            assert_eq!(mapped.find_frame(9, 0, 4, None), Some(4));
        }
