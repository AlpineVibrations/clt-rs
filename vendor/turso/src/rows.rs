use crate::turso::{assert_send_sync, Column, Error, Result, Statement, Value};
use std::fmt::Debug;
...
pub struct Row {
    pub(crate) values: Vec<crate::turso_sdk_kit::rsapi::Value>,
}
...
impl Row {
    pub fn get_value(&self, idx: usize) -> Result<Value> {
        ...
        match val {
            ...
            crate::turso_sdk_kit::rsapi::Value::Blob(items) => Ok(Value::Blob(items.to_vec())),
        }
    }

    pub fn get<T>(&self, idx: usize) -> Result<T>
    where
        T: crate::turso_sdk_kit::rsapi::FromValue,
    {
        let val = self.values.get(idx).ok_or_else(|| {
            Error::Misuse(format!(
                "column index {idx} out of bounds (row has {} columns)",
