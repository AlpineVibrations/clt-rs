...
A human may explicitly accept externally completed work by moving its session-linked task to Done while the managed journal is still `WORKING`. CLT permits that move only after transactionally proving that no worker, live session, or project lease owns the task, acquiring a short project fence, cancelling the obsolete journal, and clearing its idle resume request. The cancellation remains recorded and does not claim that CLT proved the external commit. Once the journal has sealed a manifest as `FINALIZING` or reached `PUSH-PENDING`, manual replacement remains forbidden.
