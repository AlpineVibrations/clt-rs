...
After Todo-to-Doing, ordinary one-parent user commits may advance the recorded branch before sealing. CLT preserves the original launch journal and uses the current HEAD as the manifest parent. A commit with any CLT Agent author/committer identity or a CLT-Task trailer still requires another session's exact completed-commit proof, except for CLT's exact board-only checkpoint format. A concurrent commit must not already complete the current session. Merges, branch switches, and rewritten history remain incompatible. Codex preserves concurrent changes, reconciles compatible edits, and reruns affected checks; if the parent advances after sealing, it reseals the complete reviewed payload before creating the task commit.
