...
Completion note:
COMPLETED 2026-09-11: Git-off automated Todo-to-Doing activation now saves the exact run session marker under the board lock and rejects conflicting ownership, including nested tasks. Restored the affected project's missing marker without interrupting its agent and verified selected-task Log View [LIVE] in the installed TUI after the task moved to Done. Checks: cargo fmt --all -- --check; cargo clippy --locked --all-targets --all-features -- -D warnings; cargo test --locked --all-targets --all-features (547 unit, 4 architecture, 14 CLI tests passed).
