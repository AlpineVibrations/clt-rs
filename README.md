...
The existing `c` shortcut remains deliberately narrower: select a Done task or a currently blocked Todo/Doing task whose linked Codex session is idle, then press `c` to open it interactively. It does not interrupt or take over a busy session; use `i` for an active automated session. CLT reserves the project while this interactive session is open and returns to the same board and selection afterward.
