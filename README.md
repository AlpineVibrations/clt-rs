...
On the task board, select a Done task or a currently blocked Todo/Doing task and press `c` to continue its automated run in an interactive Codex session. CLT temporarily hands the terminal to Codex, then restores the same board and selection when the interactive session exits. After an automated run, CLT appends a terminal `codex:<session-id>` marker to the task so the resume link survives task moves and wording changes. This internal marker is hidden in task lists, the TUI, and the task editor, and it is the sole task-to-session resume link. The agent database retains the session ID as run history without associating it to mutable task text. A run is reported as failed if CLT cannot persist the marker on its completed or blocked task.
