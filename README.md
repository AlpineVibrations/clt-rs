...
Moving a session-linked managed Git task from Doing to Done yourself is an explicit external-completion decision. When its journal is still `WORKING` and no worker, live session, or project lease owns it, CLT transactionally cancels the obsolete proof journal, clears the idle resume request, moves the task, and reports that external completion. Stop a live session before doing this. CLT continues to refuse manual replacement once the journal has sealed a `FINALIZING` commit manifest or entered `PUSH-PENDING`.
...
A human Done move is also the recovery escape hatch for externally completed `WORKING` tasks. CLT takes a short exact project fence, verifies that the task identity still matches and that no live owner exists, records the journal as cancelled rather than falsely claiming managed Git proof, removes its stopped or queued resume control, performs the board move, and releases the fence. This does not rewrite or adopt an external commit and does not weaken sealed `FINALIZING` or `PUSH-PENDING` transactions.
