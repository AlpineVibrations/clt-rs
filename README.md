...
When the current project is registered, the Kanban console title appends its current agent status, such as `clt Console | Agent: RUNNING`. The status follows the same runtime labels as the Agent Projects pane and remains visible when the project's agent is `OFF`. It is omitted only when the project is unregistered.
