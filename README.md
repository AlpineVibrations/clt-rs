...
Press `Tab` to toggle between the task board and the full-screen Agent Projects pane. In Agent Projects, Up/Down selects a registered project, `Enter` opens that project's task board, `Space` toggles the project `ON` or `OFF`, and `g` cycles the `GIT` column through `OFF`, `COM`, and `PUSH`. These modes disable Git automation, ask Codex to commit after a completed task, or ask Codex to commit and push. The currently open project is marked with `*`, the pane's top border shows the current local time before the daemon status, and the terminal title updates to the active project.

On the task board, select a Done task and press `c` to continue its completed automated run in an interactive Codex session. CLT temporarily hands the terminal to Codex, then restores the same board and selection when the interactive session exits. This is available for tasks completed after CLT began recording task-specific Codex session IDs; older Done tasks report that no session is recorded.
...
Press uppercase `M` from Agent Projects to open the Models page; uppercase `M`, `Tab`, or `Esc` returns to Agent Projects. The Models page keeps a catalog of providers and model targets with aligned, labeled columns. `USE` shows live availability, `FAV` marks favorites, and the separate `CLT` and `CODEX` columns identify the effective CLT-wide default and the user's Codex config default; `YES` is shown when a row has that role. When no explicit CLT override exists, CLT follows the Codex default and both columns mark the same model. Press `1` through `4` to add or enable the OpenAI, OpenRouter, Ollama, or LM Studio preset; `n` adds another Responses-compatible endpoint and `a` adds a model ID to the selected provider. Left/Right changes panes, Up/Down selects, `Space` controls whether a provider or model appears in live project selection, and `f` toggles favorite status. Favorites sort first.
