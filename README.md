...
Press uppercase `M` from Agent Projects to open the Models page; uppercase `M`, `Tab`, or `Esc` returns to Agent Projects. The Models page keeps a catalog of providers and model targets. Press `1` through `4` to add or enable the OpenAI, OpenRouter, Ollama, or LM Studio preset; `n` adds another Responses-compatible endpoint and `a` adds a model ID to the selected provider. Left/Right changes columns, Up/Down selects, `Space` controls whether a provider or model appears in live project selection, and `f` favorites a model. Favorites sort first.
