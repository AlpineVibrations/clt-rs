...
The existing `c` shortcut remains deliberately narrower: select a Done task or a currently blocked Todo/Doing task whose linked Codex session is idle, then press `c` to open it interactively with workspace-write access. When the project is otherwise idle, CLT reserves the project until you return. When another Codex task is already using the project, the selected idle session opens alongside it without interrupting the active run; both sessions can modify the same worktree. CLT still prevents the exact selected session from opening twice; use `i` to take over that session's active automated run. Both modes return to the same board and selection afterward.
